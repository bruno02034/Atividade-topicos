...
@if($lastCategoryId)
  <div class="alert alert-info">Última categoria acessada (cookie): ID {{ $lastCategoryId }}</div>
@endif
...
<table class="table table-striped">
  ...
  <tbody>
    ...
    <a href="{{ route('categorias.edit', $c->id) }}" class="btn btn-sm btn-primary">
        Editar
    </a>
    ...
    <form action="{{ route('categorias.destroy', $c->id) }}" method="POST" class="d-inline">
        @csrf
        @method('DELETE')
        <button class="btn btn-sm btn-danger" onclick="return confirm('Excluir?')">
            Excluir
        </button>
    </form>
</td>
